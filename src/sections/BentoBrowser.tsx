...
export default function BentoBrowser() {
  ...
  if (loading || !data) return (
    <section id="browse" className="relative z-10 bg-offwhite px-6 py-24 lg:px-12">
      <div className="mx-auto max-w-6xl animate-pulse">
        <div className="h-8 w-48 bg-sage/20 rounded mb-4" />
        <div className="h-12 w-96 bg-sage/10 rounded mb-8" />
        <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
           {[1,2,3].map(i => <div key={i} className="h-64 bg-sage/5 rounded-3xl" />)}
        </div>
      </div>
    </section>
  );
          ...
          {categories.map((cat) => (
            ...
            >
              <div 
                ...
                style={{
                  background: 'radial-gradient(circle at var(--mx, 50%) var(--my, 50%), rgba(255,255,255,0.3), transparent 80%)'
                } as any}
              />
